<template>
  <div>
    <section>
      <menu-component classMenu="ProductDamageEdit"></menu-component>
        ...
        <div class="cards__heading">
            <h3><i class="fas fa-barcode"></i> {{$t('editDamageProducts')}}</h3>
        </div>
            ...
            <div class="card-body py-3 p-0">
                ...
                <div class="col-lg-12">
                    <h3 class="block__title mb-lg-4">{{$t('product')}}</h3>
                        ...
                        <div class="form-group col-md-12">
                            <label for="sku" class="input__label">Status</label>
                            <br>
...
export default {
    ...
    data () {
        return {  
            ...
            closeBtn: true,  
            errors: [],
            forms: {products_demage_id:'', company:'', fulfillment_center_id:'',location_id:'', product_code:'',
                qty:'',reason:'', additional_reason:'', status:'' },
            options: [],
            optionsFulfillment: [],
            optionsStatus: ['HOLD','SALE'],
    ...
    methods: {
        submitData() {
            this.$swal({
                ...
                if (result.value) {
                    ...
                    var fulfillment_center_id   = this.forms.fulfillment_center_id;
                    var location_id             = this.forms.location_id;
                    var product_code             = this.forms.product_code;
                    
                    if(typeof  fulfillment_center_id === "object"){
                        fulfillment_center_id = this.forms.fulfillment_center_id.fulfillment_center_id
                    }

                    if(typeof  product_code === "object"){
                        product_code = this.forms.product_code.product_id
                    }

                    if(typeof  location_id === "object"){
                        location_id = this.forms.location_id.location_id
                    }
                    var formData = {
                        ...
                        additional_reason       : this.forms.additional_reason,
                    };

                        
                    const baseURI  =  this.$settings.endPoint+"/products/damage/update/"+this.forms.products_demage_id;
        ...
        fetchIt() {
            ...
            return this.$http.get(baseURI).then((response) => {
                this.optionsLocation                        = [response.data.datas.location_damage.location_description]
                this.optionsFulfillment                     = [response.data.datas.fulfillment]
                this.optionsNormalProducts                  = [response.data.datas.product]
                this.forms.products_demage_id               = response.data.datas.products_demage_id
                this.forms.company                          = response.data.datas.product.company_id
                this.forms.fulfillment_center_id            = response.data.datas.fulfillment
                this.forms.product_code                     = response.data.datas.product
                this.forms.location_id                      = response.data.datas.location_damage.location_description
                this.forms.qty                              = response.data.datas.qty
                this.forms.reason                           = response.data.datas.reason
                this.forms.additional_reason                = response.data.datas.additional_reason
